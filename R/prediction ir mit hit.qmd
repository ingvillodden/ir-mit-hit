...
line.width <- 0.2
point.size <- 0.3
col_scale <- c("#104E8B", "#8B0000") # Blue and red
...
legend <- data %>% 
  filter(period != "con", variable == "w.rel.15tt") %>% 
  mutate(period = factor(period, levels = c("mit", "hit"), 
                        labels = c("Short MIT block", "Short HIT block"))) %>%
  ggplot(aes(pre, post, 
             lty = period,
             group = period, 
             color = period, 
  ...
  geom_line() + 
  geom_point() +
  scale_linetype_manual(values = line.type_scale) +
  scale_shape_manual(values = c(17, 16)) +
  scale_color_manual(values = c("#8B0000", "#104E8B")) +
...
guides(color = guide_legend(override.aes = list(size = 3)), 
       ...
       lty = guide_legend(override.aes = list(size = 3)))
legend
...
figure5B <- data %>% 
  ...
  scale_x_continuous(limits = c(70, 88), 
                     ...
                     expand = c(0.05, 0.05)) +
  labs(x = expression("% of VO"["2max"]*"4mmol  at baseline (%) "), y = NULL) +
  theme(text=element_text(family="serif", face="plain", size=lable.size),
        legend.position = "none",
        axis.line.x = element_line(size = axis.width),
        axis.line.y = element_line(size = axis.width),
...
figure5C <- data %>% 
  ...
  scale_y_continuous(limits = c(-0.25, 1), 
                     breaks = seq(from = -0.25, to = 1, by = 0.25), 
                     expand = c(0.05, 0.05)) +
  scale_x_continuous(limits = c(125, 250), 
                     breaks = seq(from = 125, to = 250, by = 25), 
                     expand = c(0.05, 0.05)) +
  labs(x = expression("Isometric peak torque at 60 degrees ∙ sec"^-1*" (Nm) "),
...
figure5D <- data %>% 
  ...
  scale_x_continuous(limits = c(25, 55), 
                     breaks = seq(from = 25, to = 55, by = 5), 
                     expand = c(0.05, 0.05)) +
  labs(x = "Muscle thickness m. vastus lateralis (mm)", y = NULL) +
  theme(text=element_text(family="serif", face="plain", size=lable.size),
        legend.position = "none",
        axis.line.x = element_line(size = axis.width),
        axis.line.y = element_line(size = axis.width),
...
figure5.part1 <-  plot_grid(figure5A, figure5B, figure5C, figure5D,
                  align = "hv",
...
figure5 <- plot_grid(legend, figure5.part1,
           align = "vv",
           ncol = 1, rel_heights = c(0.1, 1), rel_widths = c(0.5, 1))
...
ggsave("figures/figure5.pdf", plot = figure5, width = 8.5, height = 8.5, 
       dpi = 600,
       units = "cm", device= "pdf")
...
ggsave("figures/figure6.pdf", plot = figure6, width = 8.5, height = 7, 
       dpi = 600,
       units = "cm", device= "pdf")
