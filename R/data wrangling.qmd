...
# PO 15min at pre-test in all blocks. Will be used as an independent variable later.
...
# %VO2max@4mmol at pre-test in all blocks. Will be used as an independent variable later.
...
# ge.3.fresh at pre-test in all blocks. Will be used as an independent variable later.
...
# ge.2.fresh at pre-test in all blocks. Will be used as an independent variable later.
ge.2.fresh_data <- pre_post_data %>%  
  select(id, period, timepoint, ge.2.fresh) %>% 
  filter(timepoint == "pre") %>%
  select(id, period, ge.2.fresh) %>% 
  print()

# VO2max at pre-test in all blocks. Will be used as an independent variable later.
...
# Calculating changes in body mass. Will be used as a co-variate later.
...
pre_data <- pre_intervention_data %>% # husk bmi, fat.free.mass.whole, p.body.fat
  select(id, w.rel.40tt, hbmass.kg, bv.kg, lat.avg, interm.avg) %>%
  print()
...
# MÅ ENDRE DXA OG ULTRALYD TIL dbl
