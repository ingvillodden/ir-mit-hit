...
# Combining the above data sets and creating variables relative to body mass
mutate_data <- 
  cycling_data %>% 
...
# Formatting the timepoint variable to fit later analyses
...
# Combining the formatted data sets to one data set including all pre and post measurements 
pre_post_data <- 
  pre_data_mit_con %>% 
...
# Preparing data collected before the intervention only
tt_data <- 
...
immuno_data <-
  read_excel("data/immuno_ir_data.xlsx", na = "na") %>%
  print()

# Combining the pre data sets to one pre intervention data set
pre_intervention_data <-
  tt_data %>% 
  inner_join(ultrasound_data) %>% 
  inner_join(dxa_data) %>%
  inner_join(hb_data) %>% 
  inner_join(immuno_data) %>% 
  print()
...
# In this code chunk a file combining all the vo2 session data from individual files to one file are created
...
# In this code chunk the VO2 data is loaded, new variables are created, and the VO2 data is combined with the other data from interval sessions
...
sessions_pi_mit <- sessions_pi %>% 
  filter(period == "mit") %>% 
  mutate(pi.total.kj = pi*4320) %>% # PI in kJ/sec * total duration of work intervals in sec = total PI in kJ
  group_by(id, period, session) %>% 
...
sessions_pi_hit <- sessions_pi %>% 
  filter(period == "hit") %>% 
  mutate(pi.total.kj = pi*2625) %>% # PI in kJ/sec * total duration of work intervals in sec = total PI in kJ
  group_by(id, period, session) %>% 
  summarise(pi.total.kj = mean(pi.total.kj, na.rm = TRUE)) %>%
  print()
...
# Combining all session data sets to one data set 
...
# Changing pre and post data from wide to long format, and calculating absolute changes in outcome variables
...
w.rel.max_data <- pre_post_data %>%  
  select(id, period, timepoint, w.rel.max) %>% 
  filter(timepoint == "pre") %>%
  select(id, period, w.rel.max) %>% 
  print()

w.rel.4mmol_data <- pre_post_data %>%  
  select(id, period, timepoint, w.rel.4mmol) %>% 
  filter(timepoint == "pre") %>%
  select(id, period, w.rel.4mmol) %>% 
  print()
...
peak.torque.240_data <- pre_post_data %>%  
  select(id, period, timepoint, peak.torque.240) %>% 
  filter(timepoint == "pre") %>%
  select(id, period, peak.torque.240) %>% 
  print()
...
pre_data <- pre_intervention_data %>% # Remember to specify fiber type, capillary density ect here
  mutate(lean.mass.legs = `Bein Mager masse` / 1000) %>% 
  mutate(fat.free.mass.legs = `Bein Fettfri masse` / 1000) %>% 
  select(id, w.rel.40tt, hbmass.kg, bv.kg, pv.kg, lat.avg, interm.avg, lean.mass.legs, fat.free.mass.legs, BMI, type.1.area, type.2.area, p.type.1, cd, cf, type.1.cap.fibre, type.2.cap.fibre, type.1.cap.area, type.2.cap.area) %>%
  print()
...
feeling.legs_data <- 
  read_excel("data/training_diary_ir_data.xlsx", na = "na") %>%
  select(id, period, feeling.legs) %>% 
  filter(!period %in% c("pre.con", "pre.mit", "pre.hit")) %>% 
  group_by(id, period) %>% 
  summarise(feeling.legs = mean(feeling.legs, na.rm = TRUE)) %>% 
  print()
  

# Combine all data sets to one data set that will be used for analyses
data <- responses_data %>% 
  inner_join(w.rel.15tt_data) %>%
  inner_join(w.rel.max_data) %>%
  inner_join(w.rel.4mmol_data) %>%
  inner_join(vo2.rel.max_data) %>%
  inner_join(p.vo2.4mmol_data) %>%
  inner_join(ge.3.fresh_data) %>%
  inner_join(peak.torque.60_data) %>%
  inner_join(peak.torque.240_data) %>% 
  inner_join(bodymass_data) %>%
  full_join(pre_data) %>% 
  full_join(training_data) %>%
  full_join(feeling.legs_data) %>%
  print()
