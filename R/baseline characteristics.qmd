...
# In this code chunk participant characteristics at study inclusion and onset of each training period is calculated



# Baseline characteristics for all participants (n = 44) at study inclusion

# In this code chunk each of the 44 individual participant´s very first test is selected, and then mean baseline characteristics of the participants are calculated. 
# Note that the list linking participant id and participant names is saved at a safe server according to the recommendations of the Norwegian Agency for Shared Services in Education and Research
...
# Calculating baseline characteristics at onset of CON (n=32), MIT (n=22), and HIT (n=22)
...
# Lage tabell som viser mean (SD) verider på hbmasse, kapillærer osv

baseline_intervention_histology <- pre_intervention_data %>% view()
        summarise(age.m = mean(age, na.rm = TRUE),
                  age.s = sd(age, na.rm = TRUE),
                  ...
                  type.2.cap.area.m = mean(type.2.cap.area, na.rm = TRUE),
                  type.2.cap.area.s = sd(type.2.cap.area, na.rm = TRUE),
                  
                  hbmass.kg.m = mean(hbmass.kg, na.rm = TRUE),
                  hbmass.kg.s = sd(hbmass.kg, na.rm = TRUE)) %>% 
  print()
...
# In this code chunk the participants average training history before the intervention is calculated
...
# In this code chunk the ambient temperature at all prolonged cycling tests is calculated
