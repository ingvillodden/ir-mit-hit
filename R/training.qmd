...
# In this code chunk training diary data for MIT, HIT, and CON are calculated, and differences between training blocks investigated using linear mixed models
...
# Calculating TRIMP scores and preparing training diary data for analyses
training <-
  training_diary %>% 
  ...
  filter(!period  %in% c("pre.mit", "pre.hit", "pre.con")) %>% # Removing traing data before each training period
  group_by(period) %>%
  print()

# Investigating differences in training variables between CON, MIT, and HIT using linear mixed models
...
# Making a table with mean (SD) for the given training variables in CON, MIT and HIT
...
# In this code chunk differences between MIT and HIT in interval session data is investigated using linear mixed models


# Investigating differences in interval session data between MIT and HIT using linear mixed models
# Note that there are only aerobic energy turnover and VO2 data from 14 of the 22 participants completing MIT and HIT
...
# Making a table with mean (SD) for the given interval session variables in MIT and HIT
table <- sessions_data %>% 
 group_by(period) %>% 
